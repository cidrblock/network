...
class L3_interfaces(ConfigBase, L3_interfacesArgs):
            ...
            if have_intf_vifs:
                for vif in have_intf_vifs:
                    if (vif.get('ipv4') or vif.get('ipv6')):
                        commands.append(L3_interfaces.del_cmd + have_intf_name +
                                        ' vif ' + vif['vlan_id'] + ' address')
        return commands
    ...
    def _render_updates(**kwargs):
        ...
        if have_element.get('vif'):
            set_cmd = L3_interfaces.set_cmd + intf_name + \
            ...
            if key in ['ipv4', 'ipv6'] and value:
                for item in value:
                    ...
                    if (not have_ipaddrs) or (
                        ...
                        commands.append(set_cmd + ' address \'' + item + "'")
    ...
    def _render_set_commands(**kwargs):
        commands = []
        want_element = kwargs['want_element']
        set_cmd = L3_interfaces.set_cmd + want_element['intf']['name']
    ...
    def _render_del_commands(**kwargs):
        ...
        del_cmd = L3_interfaces.del_cmd + have_element['intf']['name']
        ...
        for attrib in ['ipv4', 'ipv6']:
            ...
            if have_addr:
                for addr in set(have_addr) - set(want_addr):
                    commands.append(del_cmd + ' address \'' + addr + "'")
