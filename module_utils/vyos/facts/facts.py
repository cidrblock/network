...
class Facts(FactsArgs, FactsBase):  # pylint: disable=R0903
    ...
    def get_facts(self, module, connection, gather_subset=['!config'], gather_network_resources=['all']):
        ...
        valid_network_resources_subsets = self.argument_spec['gather_network_resources'].get(
            'choices', [])
        ...
        if valid_network_resources_subsets:
            resources_runable_subsets = self.generate_runable_subsets(
                module, gather_network_resources, valid_network_resources_subsets)
            if resources_runable_subsets:
                self.ansible_facts['gather_network_resources'] = list(
                    resources_runable_subsets)
                for attr in resources_runable_subsets:
                    getattr(self, '_get_%s' % attr, {})(module, connection)

        if self.VALID_GATHER_SUBSETS:
            runable_subsets = self.generate_runable_subsets(
                module, gather_subset, self.VALID_GATHER_SUBSETS)
    ...
    def _get_l3_interfaces(module, connection):
        return L3_interfacesFacts(L3_interfacesArgs.
